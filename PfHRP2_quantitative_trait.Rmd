...
knitr::opts_chunk$set(cache = TRUE, cache.comments = FALSE,
                      ...
                      fig.pos = 'H',dev = c('png','postscript'), dpi = 300)
library(mitools)
library(mvtnorm)
library(matrixStats)
library(missForest)
...
for(j in 1:3){ 
  ...
  res_imp[[j]] = res_imp[[j]][ind_imp, ]
    write.csv(res_imp[[j]], file = paste0('results_',my_endpoints[j],'.csv'))
...
```{r adjusted_effect_models}
res_imp_covadj = list()
for(j in 1:3){
  res_imp_covadj[[j]] = array(dim = c(length(gg_names),3))
}
...
for(cc in 1:length(gg_names)){
  ...
  for(j in 1:3){
    ...
    imp_dat_obj = mitools::imputationList(lapply(imp_dat_mod, function(x) x))
    mods = MIcombine(with(imp_dat_obj, lm(y ~ g+ethnic+agemths+bloodculture_pos+hb)))
    
    writeLines(sprintf('For %s model p=%s',
                       my_endpoints[j],
                       MIcombineP(mods)['g']))
    
    res_imp_covadj[[j]][cc,1:3] = mods$coefficients['g']+ 
      c(0,-1.96, 1.96)*sqrt(mods$variance['g','g'])
  }
}

for(j in 1:3){ 
  res_imp_covadj[[j]] = data.frame(res_imp_covadj[[j]])
  colnames(res_imp_covadj[[j]]) = c('Estimate','upperCI','lowerCI')
  rownames(res_imp_covadj[[j]]) = gg_labels
  res_imp_covadj[[j]] = res_imp_covadj[[j]][ind_imp, ]
}
```
...
layout(mat = matrix(data = c(1,4,4,2,4,4,3,4,4), nrow = 3, byrow = T))

h1=hist(kemri_case_data$log_parasites,main = '', 
     xlab = 'Parasite density (per uL)',breaks = 35,
     ylab = 'Number of patients', xaxt='n', 
     col = (mycols[1]), border = NA)
axis(1, at = 2:6, 
...
hist(kemri_case_data$log_ratio, main = '', 
     xlab = 'Ratio (ng/parasite)', breaks = 30,
     ylab = 'Number of patients', xaxt='n',
     col = (mycols[3]), border = NA)
axis(1, at = seq(-7, -1, by =2), 
     labels = c(expression(10^-7),
                expression(10^-5),   
                expression(10^-3),
                expression(10^-1)))
mtext(text = 'c',side = 3,line = 2,adj = 0, cex=1.5)
...
legend('bottomleft', inset = 0.05, 
       col=mycols, pch = 16:18, cex=1.3,
       legend = c('Parasite density','Plasma PfHRP2','PfHRP2/parasite ratio'))
mtext(text = 'd',side = 3,line = 1,adj = 0, cex=1.5)
par(font.axis = 3)
...
for(j in 1:3){
  points(res_imp_covadj[[j]]$Estimate, 1:length(gg_names) - (j-2)*0.2, 
         pch=15+j,cex=1.5, col=mycols[j])
  for(i in 1:length(gg_names)){
    lines(c(res_imp_covadj[[j]]$lowerCI[i], 
            res_imp_covadj[[j]]$upperCI[i]), 
          c(i,i) - (j-2)*0.2,lwd=2,col=mycols[j])
  }
}
...
```{r comparison_cc_imp, fig.height=6, fig.width=9}
par(mfrow=c(2,3),las=1,cex.lab=1.3, cex.axis=1.3)
my_titles = c('log parasite density',
              'log PfHRP2', 
              'log parasite/PfHRP2 ratio')
for(j in 1:3){
  xlims = range(c(range(res_cc[[j]]),range(res_imp_covadj[[j]])))
  ...
  plot(res_cc[[j]]$Estimate, res_imp[[j]]$Estimate,
       ...
       xlab='Fold change (complete case)',
       ylab="Fold change (MI)")
  axis(1, at = log10(c(.1,.2,.5,1,2)), 
     labels = c(.1,.2,.5,1,2))
  axis(2, at = log10(c(.1,.2,.5,1,2)), 
     labels = c(.1,.2,.5,1,2))
  lines(-10:10, -10:10)
  for(i in 1:length(gg_names)){
...
for(j in 1:3){
  ...
  ylims = range(res_imp[[j]])
  my_lims = range(c(xlims, ylims)) # square plot
  plot(res_imp_covadj[[j]]$Estimate, res_imp[[j]]$Estimate,
       xlim=my_lims, ylim=my_lims,main=my_titles[j],
       panel.first=grid(), xaxt='n', yaxt='n',
       xlab='Fold change (MI, adjusted)',
       ylab="Fold change (MI)")
  axis(1, at = log10(c(.1,.2,.5,1,2)), 
     labels = c(.1,.2,.5,1,2))
  axis(2, at = log10(c(.1,.2,.5,1,2)), 
  ...
  lines(-10:10, -10:10)
  for(i in 1:length(gg_names)){
    lines(rep(res_imp_covadj[[j]]$Estimate[i], 2),
          c(res_imp[[j]]$lowerCI[i],res_imp[[j]]$upperCI[i]))
    lines(c(res_imp_covadj[[j]]$lowerCI[i],res_imp_covadj[[j]]$upperCI[i]),
          rep(res_imp[[j]]$Estimate[i], 2))
  }
  abline(v=0, h=0, lty=2)
}
...
```{r fig2, echo=F}
par(mfrow = c(2,4), las=1,  cex.lab=1.3, cex.axis=1.2, bty='n')
xx=boxplot(log_parasites ~ hbb_rs334, kemri_case_data,
...
axis(2, at = 2:6, 
     labels = c(expression(10^2),
                ...
                expression(10^6)))
for(i in 1:3){
  mtext(text = paste0('n=',xx$n[i]),side = 3,at = i,cex=.7)
}
xx=boxplot(log_hrp2 ~ hbb_rs334, kemri_case_data,
...
xx=boxplot(log_parasites ~ abo_rs8176719, kemri_case_data,
        ylab='Parasite density (per/ul)',yaxt='n',
...
xx=boxplot(log_hrp2 ~ abo_rs8176719, kemri_case_data,   
        ylab='Plasma PfHRP2 (ng/ml)',yaxt='n',
        varwidth=T, xlab = expression(italic(ABO))); grid()
axis(2, at = 1:5, 
     labels = c(10,
...
xx=boxplot(log_parasites ~ hba1_2, kemri_case_data,
        ylab='Parasite density (per/ul)',yaxt='n',
...
xx=boxplot(log_hrp2 ~ hba1_2, kemri_case_data,
        ylab='Plasma PfHRP2 (ng/ml)',yaxt='n',
...
xx=boxplot(log_parasites ~ frem3_rs186873296, kemri_case_data,
        ylab='Parasite density (per/ul)',yaxt='n',
...
xx=boxplot(log_parasites ~ atp2b4_rs1541255, kemri_case_data,
        ylab='Parasite density (per/ul)',yaxt='n',
...
axis(2, at = 2:6, 
     labels = c(expression(10^2),
                ...
                expression(10^6)))
for(i in 1:3){
  mtext(text = paste0('n=',xx$n[i]),side = 3,at = i)
}

xx=boxplot(log_hrp2 ~ atp2b4_rs1541255, kemri_case_data,
        ylab='Plasma PfHRP2 (ng/ml)',yaxt='n',
        varwidth=T,xlab = expression(italic(ATP2B4))); grid()
axis(2, at = 1:5, 
     labels = c(10,
...
xx=boxplot(log_ratio ~ atp2b4_rs1541255, kemri_case_data,
...
```{r}
## load posterior distribution from previous modelling work
## see https://www.medrxiv.org/content/10.1101/2021.10.27.21265557v1
load('mod2_LC_all_cor.stanfit')
...
```{r fig4, fig.width=7, fig.height=5}
par(mar=c(5,9,4,2),las=1, cex.lab=1.3, cex.axis=1.3)
...
xx=boxplot(hb ~ as.factor(atp2b4_rs1541255_num), kemri_case_data,
